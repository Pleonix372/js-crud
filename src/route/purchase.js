...
Product.add(
  ...
  `AMD Ryzen 3600 (3.6 - 4.2 ГГц) / RAM 16 ГБ / HDD 1 ТБ + SSD 480 ГБ / nVidia GeForse RTX 3050, 8 ГБ / без ОД / LAN / без ОС`,
  [
...
Product.add(
  'https://picsum.photos/200/300',
  `Комп'ютер Artline Gaming (X43v31) Intel Xeon E-2226G/`,
...
router.post('/purchase-submit', function (req, res) {
  ...
  if (product.amount < amount) {
    return res.render('alert', {
      ...
      data: {
        message: 'Помилка',
        info: 'Товару нема в потрібній кількіості',
        link: `/purchase-list`,
      },
    })
  }
...
router.get('/purchase-order-data', function (req, res) {
  const id = Number(req.query.id)

  const purchase = Purchase.getById(id)
  console.log('purchase', purchase)

  res.render('purchase-order-data', {
    style: 'purchase-order-data',
    data: {
      id: purchase.id,
...
router.get('/purchase-order-change', function (req, res) {
  const id = Number(req.query.id)
  ...
  if (!purchase) {
    ...
    res.render('purchase-order-change', {
      style: 'purchase-order-change',
      data: {
        id: purchase.id,
        firstname: purchase.firstname,
        lastname: purchase.lastname,
...
router.post('/purchase-order-change', function (req, res) {
  const id = Number(req.query.id)
